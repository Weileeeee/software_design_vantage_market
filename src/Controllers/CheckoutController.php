...
class CheckoutController
{
    ...
    public function index()
    {
        ...
        // 2. Fetch their database-backed shopping cart
    ...
    public function processCheckout()
    {
        ...
        // Fetch user's cart from the repository
        $cart = $this->cartRepo->findOrCreateForUser($userId);
        $cartItems = $this->cartRepo->getItems($cart->cartId);
        ...
        try {
            ...
            $stmt = $this->db->prepare("INSERT INTO orders (user_id, subtotal, discount, total_amount, status) VALUES (:uid, :sub, :disc, :tot, 'pending')");
            $stmt->execute([
                'uid'  => $userId,
                'sub'  => $subtotal,
                'disc' => $discountAmount,
            ...
            foreach ($validatedItems as $item) {
                // Log the breakdown
                $stmt = $this->db->prepare("INSERT INTO order_items (order_id, product_id, quantity, price_per_unit) VALUES (:oid, :pid, :qty, :price)");
                $stmt->execute([
                    'oid'   => $newOrderId,
                    'pid'   => $item['product_id'],
                    'qty'   => $item['quantity'],
            ...
            // C. Clean out the shopping cart table directly via Database query 
            // (Used directly in case the Repo doesn't have a clearCart method yet)
            ...
            // Redirect to success/tracking interface
