...
class OrderRepository
{
    ...
    public function createFromCart(int $cartId, ?int $userId, ?string $sessionId): Order
    {
        ...
        // Insert order header (address_id=1 default; session_id stored for guest tracking)
        $this->db->prepare(
            'INSERT INTO Orders (user_id, address_id, total_amount, status)
             VALUES (:user_id, 1, :total, :status)'
        )->execute([
            ':user_id' => $userId,
            ':total'   => $total,
            ':status'  => 'pending',
        ...
        $itemStmt = $this->db->prepare(
            'INSERT INTO Order_Items (order_id, product_id, quantity, purchased_price)
    ...
    {
        // If session_id column doesn't exist (main schema), return empty array gracefully
        try {
            $stmt = $this->db->prepare(
                'SELECT * FROM Orders
            ...
            return []; // session_id column may not exist in this schema version
        }
    }
    ...
    public function getItems(int $orderId): array
    {
        $stmt = $this->db->prepare(
            'SELECT oi.order_item_id,
                    ...
                    oi.product_id,
                    oi.quantity,
                    oi.purchased_price AS price_at_purchase,
                    p.title,
                    p.sku
             FROM   Order_Items oi
